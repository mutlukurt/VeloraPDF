...
export function PdfThumbnailPanel({ pdf }: { pdf: PDFDocumentProxy }) {
  ...
  const pages = Array.from({ length: pageCount }, (_, index) => index + 1).filter((page) => !filter || String(page).includes(filter));

  return (
    <aside className="w-60 shrink-0 overflow-y-auto border-r border-border bg-sidebar p-3">
